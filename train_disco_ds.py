...
def parse_args():
    parser = argparse.ArgumentParser(description="Full fine-tuning script for DisCo (DeepSpeed single-model).")
    ...
    parser.add_argument("--tracker_project_name", type=str, default="text2image_fullft_ds")
...
def precompute_stats_and_priors(args):
    ...
    priors_path = os.path.join(args.output_dir, "box_priors.pt")

    if is_main:
        print("[precompute] collect_data_statistics:start")
        tokenizer = CLIPTokenizer.from_pretrained(args.pretrained_diffusion_model_path, subfolder="tokenizer")
        train_dataloader, _, _, vocab = build_train_dataloader(args, tokenizer=tokenizer)
        num_objs = len(vocab['object_idx_to_name'])
        num_rels = len(vocab['pred_idx_to_name'])
        image_obj_idx = vocab['object_name_to_idx'].get('__image__', 0)
        sl_vae = SceneVAEModel(args, num_objs, num_rels, image_obj_idx=image_obj_idx)
        device_cpu = torch.device("cpu")
        collate = getattr(train_dataloader, 'collate_fn', None)
        ...
        box_mean_est, box_cov_est = sl_vae.collect_data_statistics(tmp_loader, device_cpu)
        tmp_loader = None
        torch.cuda.empty_cache()
        mean_t = torch.as_tensor(box_mean_est)
        cov_t = torch.as_tensor(box_cov_est)
        Path(stats_path).parent.mkdir(parents=True, exist_ok=True)
        torch.save({"mean": mean_t.cpu(), "cov": cov_t.cpu()}, stats_path)
        print(f"[precompute] collect_data_statistics:done -> {stats_path}")

        print("[precompute] collect_class_priors:start")
        priors = {}
        counts = {}
        collate = getattr(train_dataloader, 'collate_fn', None)
        tmp_loader = torch.utils.data.DataLoader(
        ...
        for batch in tqdm(tmp_loader, desc="collect_class_priors"):
        ...
        print(f"[precompute] collect_class_priors:done -> {priors_path}")
    else:
        print("[precompute] waiting for stats/priors files...")
        for _ in range(60):
            if os.path.exists(stats_path) and os.path.exists(priors_path):
                break
            time.sleep(1)
        print("[precompute] files ready")
...
class TrainerDS:
    def __init__(self, args):
        ...
        self.same_overlap_weight = 0.1
        self.category_prior_weight = 0.1
        self.use_angle_prior = False

        fusion_params = list(self.model.fusion.parameters())
        sl_vae_params = list(self.model.sl_vae.parameters())
        base_unet_params = list(self.model.unet.parameters())
        ds_param_groups = [
            dict(params=base_unet_params, lr=args.learning_rate),
        ]
        ...
        if args.use_ema:
            ...
            self.ema_unet = EMAModel(self.model.unet.parameters(), model_cls=UNet2DConditionModel, model_config=unet_native.config)
...
if __name__ == '__main__':
    args = parse_args()
    precompute_stats_and_priors(args)
