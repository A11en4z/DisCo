...
class SceneVAEModel(nn.Module):
    ...
    def __init__(self, args, num_objs, num_rels):
        ...
        # )
        self.mlp_box = build_mlp(
            [args.embedding_dim * 2 + 512, gconv_hidden_dim, 6],  # 水平:4,角度直接回归：5,cos/sin:6
            batch_norm="batch", 
            final_nonlinearity=False
        )
    ...
    def decoder(self, objs, obj_clip_embs, z, triples, rel_clip_embs):
        ...
        # # 对前4维做 sigmoid（如果你使用归一化坐标）
        # box_pred[..., :4] = torch.sigmoid(box_pred[..., :4])
        # # 强制 __image__ angle 为 0
        # box_pred[-1, 4] = 0.0
        ...
        angle_vec_normalized  = angle_vec / angle_norm
        
        # sigmoid 归一化中心与尺寸
        box_xywh = torch.sigmoid(box_pred[..., :4])  # ∈ (0,1)
        box_pred = torch.cat([box_xywh, angle_vec_normalized], dim=-1)

        # 强制 __image__ 的角度为 0，即 cosθ=1, sinθ=0 ，并写死位置
        box_pred = box_pred.clone()
        box_pred[-1, :4] = torch.tensor([0.5, 0.5, 1.0, 1.0], device=box_pred.device)
        box_pred[-1, 4:6] = torch.tensor([1.0, 0.0], device=box_pred.device)
