...
class ObjectFusionTokenizer(nn.Module):
    ...
    def prepare_data(self, boxes, text_embeddings, obj_to_img):
        ...
        masks = torch.zeros(B, max_objs, device=device, dtype=text_embeddings.dtype)
        text_out = torch.zeros(B, max_objs, 768, device=device, dtype=text_embeddings.dtype)
        # 保留角度维度：5D [cx, cy, w, h, angle]
        boxes_in = boxes[..., :5]
        box_out = torch.zeros(B, max_objs, 5, device=device, dtype=boxes.dtype)
