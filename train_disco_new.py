...
class Trainer:
    def __init__(self, args):
        ...
        base_root = args.pretrained_diffusion_model_path
        if args.resume_from is not None and os.path.isdir(args.resume_from):
            cfg_path = os.path.join(args.resume_from, "config.json")
            if os.path.exists(cfg_path):
                try:
                    with open(cfg_path, "rt") as f:
                        cfg = json.load(f)
                    pr = cfg.get("pretrained_diffusion_model_path", None)
                    if isinstance(pr, str) and os.path.isdir(pr):
                        base_root = pr
                except Exception:
                    pass
        self.noise_scheduler = DDPMScheduler.from_pretrained(base_root, subfolder="scheduler")
        self.scheduler = PNDMScheduler.from_pretrained(base_root, subfolder="scheduler")
        self.tokenizer = CLIPTokenizer.from_pretrained(base_root, subfolder="tokenizer")
        ...
        with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
            self.text_encoder = CLIPTextModel.from_pretrained(base_root, subfolder="text_encoder")
            self.vae = AutoencoderKL.from_pretrained(base_root, subfolder="vae")

        self.unet = UNet2DConditionModel.from_pretrained(base_root, subfolder="unet")
        # 安装自定义跨注意力处理器（无LoRA），使object_embeddings生效
        register_attention_control(self.unet)
        if args.resume_from is not None and os.path.isdir(args.resume_from):
            try:
                from safetensors.torch import load_file as safetensors_load
                unet_sd_path = os.path.join(args.resume_from, "unet", "diffusion_pytorch_model.safetensors")
                if os.path.exists(unet_sd_path):
                    sd = safetensors_load(unet_sd_path)
        ...
        if args.resume_from is not None and os.path.isdir(args.resume_from):
            sl_path = os.path.join(args.resume_from, "sl_vae.pt")
            fu_path = os.path.join(args.resume_from, "fusion.pt")
            if os.path.exists(sl_path):
                self.sl_vae.load_state_dict(torch.load(sl_path, map_location="cpu"))
            if os.path.exists(fu_path):
                self.object_fusion_tokenizer.load_state_dict(torch.load(fu_path, map_location="cpu"))

        stats_path = os.path.join(args.output_dir, "box_stats.pt")
        resume_stats = os.path.join(args.resume_from, "box_stats.pt") if args.resume_from else None
        stats_load_path = resume_stats if (resume_stats and os.path.exists(resume_stats)) else stats_path
        if not (resume_stats and os.path.exists(resume_stats)):
            if self.accelerator.is_main_process:
        ...
        param_groups = [
            dict(params=unet_params, lr=args.learning_rate),
        ...
        self.resume_step = 0
        if args.resume_from is not None and os.path.isdir(args.resume_from):
            val_dir = os.path.join(args.resume_from, "validation")
        ...
        stats = torch.load(stats_load_path, map_location="cpu")
        self.box_mean_est = stats["mean"].numpy()
        self.box_cov_est = stats["cov"].numpy()

        priors_path = os.path.join(self.args.output_dir, "box_priors.pt")
        resume_priors = os.path.join(self.args.resume_from, "box_priors.pt") if self.args.resume_from else None
        if resume_priors and os.path.exists(resume_priors):
            self.box_priors = torch.load(resume_priors, map_location="cpu")
        else:
            if self.accelerator.is_main_process:
                priors = {}
                counts = {}
                for batch in self.train_dataloader:
                    ...
                    classes = objs.long()
                    for ci in classes.unique().tolist():
                        mask = (classes == ci)
                        feats_ci = feats[mask]
                        if feats_ci.shape[0] == 0:
                            continue
                        mean = feats_ci.mean(dim=0)
                        diff = feats_ci - mean
                        cov = (diff.T @ diff) / max(1, feats_ci.shape[0] - 1)
                        if ci in priors:
                            prev = priors[ci]
                            priors[ci] = {"mean": (prev["mean"] + mean) / 2.0, "cov": (prev["cov"] + cov) / 2.0}
                        else:
                            priors[ci] = {"mean": mean, "cov": cov}
                        counts[ci] = counts.get(ci, 0) + feats_ci.shape[0]
                torch.save(priors, priors_path)
                self.box_priors = priors
            else:
                self.box_priors = None
            self.accelerator.wait_for_everyone()
            if self.box_priors is None:
                self.box_priors = torch.load(priors_path, map_location="cpu")

        if args.use_ema:
            # 多卡时 self.unet 被 DDP 包装，需先取原生模型以访问 config
        ...
        self.total_train_steps = self.resume_step + self.args.max_train_steps
        self.progress_bar = tqdm(range(0, self.total_train_steps), initial=self.resume_step, desc="Steps", disable=not self.accelerator.is_local_main_process)
    ...
    def start(self):
        ...
        if self.accelerator.is_main_process:
            ...
            train_root = os.path.dirname(self.args.output_dir)
            marker_path = os.path.join(train_root, f"{self.args.tracker_project_name}-latest.txt")
            try:
                with open(marker_path, 'wt') as mf:
                    mf.write(self.args.output_dir)
            except Exception:
                pass
        ...
        for step, batch in enumerate(self.train_dataloader):
            with self.accelerator.accumulate(self.unet):
                ...
                if self.class_weights is None:
                    ...
                    for cname in ["airplane", "Expressiveway-Service-area"]:
                        idx = self.vocab['object_name_to_idx'].get(cname, None)
                        if idx is not None:
                            self.class_weights[idx] = 2.0
                box_loss = self.box_criterion(layout_pred, layout, objs=objs.to(self.accelerator.device), class_weights=self.class_weights)
    ...
    def log_validation(self, step, ref_batch):
        ...
        if self.args.use_ema and hasattr(self, 'ema_unet'):
            unet_native = self.accelerator.unwrap_model(self.unet)
            self.ema_unet.copy_to(unet_native.parameters())
        else:
            unet_native = self.accelerator.unwrap_model(self.unet)
